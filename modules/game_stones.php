...
function game_stones_dohook($hookname, $args){
	if ($hookname=="darkhorsegame"){
		...
		output::addnav("S?Play Stones Game",
...
function game_stones_run(){
	...
	if (!isset($stones['side']) || $stones['side']==""){
		output::doOutput("`3The old man explains his game, \"`7I have a bag with 6 red stones, and 10 blue stones in it.  You can choose between 'like pair' or 'unlike pair.'  I will then draw out pairs of stones two at a time.  If they are the same color as each other, they go to which ever of us is 'like pair,' and otherwise they go to which ever of us is 'unlike pair.'  Whoever has the most stones at the end will win.  If we have the same number, then it is a draw, and no one wins.`3\"");
		output::addnav("Never Mind", appendlink(urldecode($ret), "op=oldman"));
		output::addnav("Like Pair",
				"runmodule.php?module=game_stones&side=likepair&ret=$ret");
		output::addnav("Unlike Pair",
		...
		output::doOutput("`3\"`7%s pair for you, and %s pair for me it is then!  How much do you bet?`3\"", $s1, $s2);
		...
		output::addnav("","runmodule.php?module=game_stones&ret=$ret");
		output::addnav("Never Mind", appendlink(urldecode($ret), "op=oldman"));
		...
		output::doOutput("`3The old man reaches into his bag and withdraws two stones.");
		output::doOutput("They are %s and %s.  Your bet is `^%s`3.`n`n", $s1, $s2, $stones['bet']);
		...
		output::doOutput("Since you are %s pairs, the old man places the stones in %s pile.`n`n", translator::translate_inline($stones['side']=="likepair"?"like":"unlike"), $winner);

		output::doOutput("You currently have `^%s`3 stones in your pile, and the old man has `^%s`3 stones in his.`n`n", $stones['player'], $stones['oldman']);
		output::doOutput("There are %s %s stones and %s %s stones in the bag yet.", $stones['red'], $rstone, $stones['blue'], $bstone);
		output::addnav("Continue","runmodule.php?module=game_stones&ret=$ret");
	}else{
		if ($stones['player']>$stones['oldman']){
			output::doOutput("`3Having defeated the old man at his game, you claim your `^%s`3 gold.", $stones['bet']);
			$session['user']['gold']+=$stones['bet'];
			debuglog("won {$stones['bet']} gold in the stones game");
		}elseif ($stones['player']<$stones['oldman']){
			output::doOutput("`3Having defeated you at his game, the old man claims your `^%s`3 gold.", $stones['bet']);
			$session['user']['gold']-=$stones['bet'];
			debuglog("lost {$stones['bet']} gold in the stones game");
		}else{
			output::doOutput("`3Having tied the old man, you call it a draw.");
		}
		$stones=array();
		output::addnav("Play again?","runmodule.php?module=game_stones&ret=$ret");
		output::addnav("Other Games",appendlink(urldecode($ret), "op=oldman"));
		output::addnav("Return to Main Room", appendlink(urldecode($ret), "op=tavern"));
	}
