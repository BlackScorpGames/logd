...
if ($op==""){
	if (!http::httpget('nointro')) {
		output::doOutput("`\$Fighting down every urge to flee, you cautiously enter the cave entrance, intent on catching the great green dragon sleeping, so that you might slay it with a minimum of pain.");
		output::doOutput("Sadly, this is not to be the case, for as you round a corner within the cave you discover the great beast sitting on its haunches on a huge pile of gold, picking its teeth with a rib.");
	}
	$badguy = array(
	...
	output::doOutput("`@Victory!`n`n");
	$flawless = (int)(http::httpget('flawless'));
  	if ($flawless) {
		output::doOutput("`b`c`&~~ Flawless Fight ~~`0`c`b`n`n");
	}
	output::doOutput("`2Before you, the great dragon lies immobile, its heavy breathing like acid to your lungs.");
	output::doOutput("You are covered, head to toe, with the foul creature's thick black blood.");
	output::doOutput("The great beast begins to move its mouth.  You spring back, angry at yourself for having been fooled by its ploy of death, and watch for its huge tail to come sweeping your way.");
	output::doOutput("But it does not.");
	output::doOutput("Instead the dragon begins to speak.`n`n");
	output::doOutput("\"`^Why have you come here mortal?  What have I done to you?`2\" it says with obvious effort.");
	output::doOutput("\"`^Always my kind are sought out to be destroyed.  Why?  Because of stories from distant lands that tell of dragons preying on the weak?  I tell you that these stories come only from misunderstanding of us, and not because we devour your children.`2\"");
	output::doOutput("The beast pauses, breathing heavily before continuing, \"`^I will tell you a secret.  Behind me now are my eggs.  They will hatch, and the young will battle each other.  Only one will survive, but she will be the strongest.  She will quickly grow, and be as powerful as me.`2\"");
	output::doOutput("Breath comes shorter and shallower for the great beast.`n`n");
	output::doOutput("\"`#Why do you tell me this?  Don't you know that I will destroy your eggs?`2\" you ask.`n`n");
	output::doOutput("\"`^No, you will not, for I know of one more secret that you do not.`2\"`n`n");
	output::doOutput("\"`#Pray tell oh mighty beast!`2\"`n`n");
	output::doOutput("The great beast pauses, gathering the last of its energy.  \"`^Your kind cannot tolerate the blood of my kind.  Even if you survive, you will be a feeble creature, barely able to hold a weapon, your mind blank of all that you have learned.  No, you are no threat to my children, for you are already dead!`2\"`n`n");
	output::doOutput("Realizing that already the edges of your vision are a little dim, you flee from the cave, bound to reach the healer's hut before it is too late.");
	output::doOutput("Somewhere along the way you lose your weapon, and finally you trip on a stone in a shallow stream, sight now limited to only a small circle that seems to float around your head.");
	output::doOutput("As you lay, staring up through the trees, you think that nearby you can hear the sounds of the village.");
	output::doOutput("Your final thought is that although you defeated the dragon, you reflect on the irony that it defeated you.`n`n");
	output::doOutput("As your vision winks out, far away in the dragon's lair, an egg shuffles to its side, and a small crack appears in its thick leathery skin.");
	...
	output::doOutput("`n`nYou wake up in the midst of some trees.  Nearby you hear the sounds of a village.");
	output::doOutput("Dimly you remember that you are a new warrior, and something of a dangerous Green Dragon that is plaguing the area.  You decide you would like to earn a name for yourself by perhaps some day confronting this vile creature.");

	// allow explanative text as well.
	modulehook("dragonkilltext");

	$regname = get_player_basename();
	output::doOutput("`n`n`^You are now known as `&%s`^!!",$session['user']['name']);
	if ($session['user']['dragonkills'] == 1) {
		...
		output::doOutput("`n`n`&Because you have slain the dragon %s time, you start with some extras.  You also keep additional permanent hitpoints you've earned.`n",$session['user']['dragonkills']);
	} else {
		...
		output::doOutput("`n`n`&Because you have slain the dragon %s times, you start with some extras.  You also keep additional permanent hitpoints you've earned.`n",$session['user']['dragonkills']);
	}
	$session['user']['charm']+=5;
	output::doOutput("`^You gain FIVE charm points for having defeated the dragon!`n");
...
if ($op=="run"){
	output::doOutput("The creature's tail blocks the only exit to its lair!");
	$op="fight";
	httpset('op', 'fight');
}
if ($op=="fight" || $op=="run"){
...
if ($battle){
	...
	if ($victory){
		...
		if($defeat){
			...
			output::doOutput("`b`&You have been slain by `@The Green Dragon`&!!!`n");
