...
if ($op==""){
  checkday();
 output::doOutput("`6As you approach the pair of impressive carved rock crystal doors, they part to allow you entrance into the bank.");
 output::doOutput("You find yourself standing in a room of exquisitely vaulted ceilings of carved stone.");
 output::doOutput("Light filters through tall windows in shafts of soft radiance.");
 output::doOutput("About you, clerks are bustling back and forth.");
 output::doOutput("The sounds of gold being counted can be heard, though the treasure is nowhere to be seen.`n`n");
 output::doOutput("You walk up to a counter of jet black marble.`n`n");
 output::doOutput("`@Elessa`6, a petite woman in an immaculately tailored business dress, greets you from behind reading spectacles with polished silver frames.`n`n");
 output::doOutput("`6\"`5Greetings, my good lady,`6\" you greet her, \"`5Might I inquire as to my balance this fine day?`6\"`n`n");
 output::doOutput("`@Elessa`6 blinks for a moment and then smiles, \"`@Hmm, `&%s`@, let's see.....`6\" she mutters as she scans down a page in her ledger.",$session['user']['name']);
	if ($session['user']['goldinbank']>=0){
		output::doOutput("`6\"`@Aah, yes, here we are.  You have `^%s gold`@ in our prestigious bank.  Is there anything else I can do for you?`6\"",$session['user']['goldinbank']);
	}else{
		output::doOutput("`6\"`@Aah, yes, here we are.  You have a `&debt`@ of `^%s gold`@ in our prestigious bank.  Is there anything else I can do for you?`6\"",abs($session['user']['goldinbank']));
	}
}elseif($op=="transfer"){
	...
	if ($session['user']['goldinbank']>=0){
		...
		output::doOutput("Transfer how much: ");
	...
	output::doOutput("`6`bConfirm Transfer`b:`n");
	$string="%";
	$to = httppost('to');
	...
	$amt = abs((int)httppost('amount'));
	$to = httppost('to');
	output::doOutput("`6`bTransfer Completion`b`n");
	if ($session['user']['gold']+$session['user']['goldinbank']<$amt){
		output::doOutput("`@Elessa`6 stands up to her full, but still diminutive height and glares at you, \"`@How can you transfer `^%s`@ gold when you only possess `^%s`@?`6\"",$amt,$session['user']['gold']+$session['user']['goldinbank']);
	}else{
		...
		if (db_num_rows($result)==1){
			...
			if ($session['user']['amountouttoday']+$amt > $maxout) {
				...
				output::doOutput("`@Elessa`6 glares at you, her eyes flashing dangerously, \"`@You may not transfer money to yourself!  That makes no sense!`6\"");
			}else{
				...
				db_query($sql);
				output::doOutput("`@Elessa`6 smiles, \"`@The transfer has been completed!`6\"");
			...
			output::doOutput("`@Elessa`6 looks up from her ledger with a bit of surprise on her face, \"`@I'm terribly sorry, but I seem to have run into an accounting error, would you please try telling me what you wish to transfer again?`6\"");
		}
	}
}elseif($op=="deposit"){
	output::doOutput("`0");
	...
	output::doOutput("`6Searching through all your pockets and pouches, you calculate that you currently have `^%s`6 gold on hand.`n`n", $session['user']['gold']);
  ...
  addnav("","bank.php?op=depositfinish");
	...
	output_notl($session['user']['goldinbank']>=0?$balance:$debt,abs($session['user']['goldinbank']));
	output::doOutput("`6\"`@How much would you like to borrow `&%s`@?  At your level, you may borrow up to a total of `^%s`@ from the bank.`6\"`n`n",$session['user']['name'], $maxborrow);
	...
	rawoutput("<form action='bank.php?op=withdrawfinish' method='POST'>");
	output_notl($session['user']['goldinbank']>=0?$balance:$debt,abs($session['user']['goldinbank']));
	output::doOutput("`6\"`@How much would you like to withdraw `&%s`@?`6\"`n`n",$session['user']['name']);
	...
	if ($amount>$session['user']['goldinbank'] && httppost('borrow')=="") {
		output::doOutput("`\$ERROR: Not enough gold in the bank to withdraw.`^`n`n");
		output::doOutput("`6Having been informed that you have `^%s`6 gold in your account, you declare that you would like to withdraw all `^%s`6 of it.`n`n", $session['user']['goldinbank'], $amount);
		output::doOutput("`@Elessa`6 looks at you for a few moments without blinking, then advises you to take basic arithmetic.  You realize your folly and think you should try again.");
		...
		if ($lefttoborrow<=$session['user']['goldinbank']+$maxborrow){
			...
			if ($lefttoborrow-$session['user']['goldinbank'] > $maxborrow){
				if ($didwithdraw) {
					output::doOutput("`6Additionally, you ask to borrow `^%s`6 gold.", $lefttoborrow);
				} else {
					output::doOutput("`6You ask to borrow `^%s`6 gold.", $lefttoborrow);
				}
				output::doOutput("`@Elessa`6 looks up your account and informs you that you may only borrow up to `^%s`6 gold.", $maxborrow);
			}else{
				if ($didwithdraw) {
					output::doOutput("`6Additionally, you borrow `^%s`6 gold.", $lefttoborrow);
				} else {
					output::doOutput("`6You borrow `^%s`6 gold.", $lefttoborrow);
				}
				$session['user']['goldinbank']-=$lefttoborrow;
				$session['user']['gold']+=$lefttoborrow;
				debuglog("borrows $lefttoborrow gold from the bank");
				output::doOutput("`@Elessa`6 records your withdrawal of `^%s `6gold in her ledger. \"`@Thank you, `&%s`@.  You now have a debt of `\$%s`@ gold to the bank and `^%s`@ gold in hand.`6\"", $amount,$session['user']['name'], abs($session['user']['goldinbank']),$session['user']['gold']);
			}
		}else{
			output::doOutput("`6Considering the `^%s`6 gold in your account, you ask to borrow `^%s`6. `@Elessa`6 peers through her ledger, runs a few calculations and then informs you that, at your level, you may only borrow up to a total of `^%s`6 gold.", $session['user']['goldinbank'], $lefttoborrow-$session['user']['goldinbank'], $maxborrow);
		}
	}else{
		$session['user']['goldinbank']-=$amount;
		$session['user']['gold']+=$amount;
		debuglog("withdrew $amount gold from the bank");
		output::doOutput("`@Elessa`6 records your withdrawal of `^%s `6gold in her ledger. \"`@Thank you, `&%s`@.  You now have a balance of `^%s`@ gold in the bank and `^%s`@ gold in hand.`6\"", $amount,$session['user']['name'], abs($session['user']['goldinbank']),$session['user']['gold']);
	}
}
villagenav();
addnav("Money");
