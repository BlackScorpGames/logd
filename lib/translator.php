...
class translator {
    ...
    public static function translate($indata,$namespace=FALSE){
            ...
            if ($namespace != "notranslate") {
                    if (!isset($translation_table[$namespace]) ||
                            ...
                            $translation_table[$namespace] =
                                    translator::translate_loadnamespace($namespace,(isset($session['tlanguage'])?$session['tlanguage']:false));
                    }
            }
            ...
            if (is_array($indata)){
                    ...
                    while (list($key,$val)=each($indata)){
                            $outdata[$key] = translator::translate($val,$namespace);
                    }
            }else{
                    if ($namespace != "notranslate") {
                            ...
                            translator::tlbutton_push($indata,!$foundtranslation,$namespace);
                    } else {
                            $outdata = $indata;
                    }
            }
    ...
    public static function translate_inline($in,$namespace=FALSE){
            $out = translator::translate($in,$namespace);
    ...
    public static function tlbutton_clear(){
            global $translatorbuttons,$session;
            if ($session['user']['superuser'] & SU_IS_TRANSLATOR){
                    $return = translator::tlbutton_pop().join("",$translatorbuttons);
