...
function backtrace_getType($in){
	...
	if (is_string($in)){
		...
		if (strlen($in) > 25){
			$return.=htmlentities(substr($in,0,25)."...", ENT_COMPAT, settings::getsetting("charset", "ISO-8859-1"));
		}else{
			$return.=htmlentities($in, ENT_COMPAT, settings::getsetting("charset", "ISO-8859-1"));
		}
		$return.="\"</span>";
	}elseif (is_bool($in)){
