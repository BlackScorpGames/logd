...
output::doOutput("`n`n`^Welcome to the Legend of the Green Dragon New Player Primer`n`n");
output::doOutput("`^`bThe village square`b`n");
output::doOutput("`@Legend of the Green Dragon (LotGD) is turning out to be a fairly expansive game, with a lot of areas to explore.");
output::doOutput("It's easy to get lost with all that there is to do out there, so keep in mind that the village square is pretty much the center of the village you start in.");
output::doOutput("This area will give you access to most other areas that you can get to, with a few exceptions (we'll talk about those in a little while).");
output::doOutput("If you ever get lost, or are not sure what's going on, head to the village square and regain your bearings.`n`n");
output::doOutput("`^`bYour first day`b`n");
output::doOutput("`@Your first day in the world can be very confusing!");
output::doOutput("You're presented with a lot of information, and you don't need almost any of it!  It's true!");
output::doOutput("One thing you should probably keep an eye on though, are your hit points.  This is found under \"Vital Info.\"");
output::doOutput("No matter what profession you choose, in the end, you are some kind of warrior or fighter, and so you need to learn how to do battle.");
output::doOutput("The best way to do this is to look for creatures to kill in the forest.");
output::doOutput("When you find one, check it out, and make sure that it's not a higher level than you, because if it is, you might not live through the fight.");
output::doOutput("Keep in mind that you can always try to run away from something that you encountered, but sometimes it might take several tries before you get away.");
output::doOutput("You might want to buy armor and weapons in the village square in order to give yourself a better chance against these creatures out in the forest.`n`n");
output::doOutput("Once you have defeated a creature, you'll notice that you're probably a little hurt.");
output::doOutput("Head on over to the Healer's Hut, and you can get patched up in short order.");
output::doOutput("While you're level 1, healing is free, but as you advance, it becomes more and more expensive.");
output::doOutput("Also keep in mind that it's more expensive to heal 1 point, then later heal 1 point again than it is to heal 2 in one shot.");
output::doOutput("So if you're trying to save up some money, and you're barely hurt, you might risk a fight or two while you're a little hurt, and heal the damage from several fights in one shot.`n`n");
...
output::doOutput("`@Death is a natural part of any game that contains some kind of combat.");
...
if (getsetting("pvp",1)){
	...
	output::doOutput("Quitting to the fields means that someone can attack you without having to pay money or gems to the bartender.");
	output::doOutput("You cannot be attacked while you are online, only while you are offline, so the more  you play, the more protected you are ;-).");
	output::doOutput("Also, if you are attacked and die, no one else can attack you again until you log on again, so don't worry that you'll be attacked 30 or 40 times in one night.");
	output::doOutput("Logging back into the game will make you a viable PvP target again if you've already been killed today.`n`n");
}
output::doOutput("`^`bReady to take on the world!`b`n");
output::doOutput("`@You should now have a pretty good idea of how the basics of the game work, how to advance, and how to protect yourself.");
output::doOutput("There's a whole lot more to the world, so explore it!");
output::doOutput("Don't be afraid of dying, particularly when you're young, as even when you're dead, there's yet more stuff to do!");
