...
output::doOutput("`@`c`bDatabase Connection Information`b`c`2");
output::doOutput("In order to run Legend of the Green Dragon, your server must have access to a MySQL database.");
output::doOutput("If you are not sure if you meet this need, talk to server's Internet Service Provider (ISP), and make sure they offer MySQL databases.");
output::doOutput("If you are running on your own machine or a server under your control, you can download and install MySQL from <a href='http://www.mysql.com/' target='_blank'>the MySQL website</a> for free.`n",true);
if (file_exists("dbconnect.php")){
	output::doOutput("There appears to already be a database setup file (dbconnect.php) in your site root, you can proceed to the next step.");
}else{
	output::doOutput("`nIt looks like this is a new install of Legend of the Green Dragon.");
	output::doOutput("First, thanks for installing LoGD!");
	output::doOutput("In order to connect to the database server, I'll need the following information.");
	output::doOutput("`iIf you are unsure of the answer to any of these questions, please check with your server's ISP, or read the documentation on MySQL`i`n");

	output::doOutput("`nWhat is the address of your database server?`n");
	rawoutput("<input name='DB_HOST' value=\"".htmlentities($session['dbinfo']['DB_HOST'], ENT_COMPAT, settings::getsetting("charset", "ISO-8859-1"))."\">");
	...
	output::doOutput("`nWhat is the username you use to connect to the database server?`n");
	rawoutput("<input name='DB_USER' value=\"".htmlentities($session['dbinfo']['DB_USER'], ENT_COMPAT, settings::getsetting("charset", "ISO-8859-1"))."\">");
	...
	output::doOutput("`nWhat is the password for this username?`n");
	rawoutput("<input name='DB_PASS' value=\"".htmlentities($session['dbinfo']['DB_PASS'], ENT_COMPAT, settings::getsetting("charset", "ISO-8859-1"))."\">");
	...
	output::doOutput("`nWhat is the name of the database you wish to install LoGD in?`n");
	rawoutput("<input name='DB_NAME' value=\"".htmlentities($session['dbinfo']['DB_NAME'], ENT_COMPAT, settings::getsetting("charset", "ISO-8859-1"))."\">");
	...
	output::doOutput("`nDo you want to use datacaching (high load optimization)?`n");
	rawoutput("<select name='DB_USEDATACACHE'>");
	rawoutput("<option value=\"1\" ".($session['dbinfo']['DB_USEDATACACHE']?'selected=\"selected\"':'').">".translator::translate_inline("Yes")."</option>");
	rawoutput("<option value=\"0\" ".(!$session['dbinfo']['DB_USEDATACACHE']?'selected=\"selected\"':'').">".translator::translate_inline("No")."</option>");
	rawoutput("</select>");
	...
	output::doOutput("`nIf yes, what is the path to the datacache directory?`n");
	rawoutput("<input name='DB_DATACACHEPATH' value=\"".htmlentities($session['dbinfo']['DB_DATACACHEPATH'], ENT_COMPAT, settings::getsetting("charset", "ISO-8859-1"))."\">");
	...
	/*
		...
		output::doOutput("`nShould I attempt to create this database if it does not exist?`n");
