...
if (file_exists("dbconnect.php")){
	...
	$initial = true;
	output::doOutput("`@`c`bWriting your dbconnect.php file`b`c");
	output::doOutput("`2I'm attempting to write a file named 'dbconnect.php' to your site root.");
	output::doOutput("This file tells LoGD how to connect to the database, and is necessary to continue installation.`n");
	$dbconnect =
	"<?php\n"
	...
	if ($fp){
		if (fwrite($fp, $dbconnect)!==false){
			output::doOutput("`n`@Success!`2  I was able to write your dbconnect.php file, you can continue on to the next step.");
		}else{
			$failure=true;
		}
		fclose($fp);
...
if ($success && !$initial){
	...
	if ($sub < 110) {
		...
		if ($fp){
			if (fwrite($fp, $dbconnect)!==false){
				output::doOutput("`n`@Success!`2  I was able to write your dbconnect.php file.");
			}else{
				$failure=true;
			}
			fclose($fp);
		...
		if ($failure) {
			...
			output::doOutput("`n`^You are ready for the next step.");
		}
	} else {
		output::doOutput("`n`^You are ready for the next step.");
	}
