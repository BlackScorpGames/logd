	...
	if (isset($userinfo[$key])){
		if ($key=="newpassword" ){
			if ($val>"") {
				...
				$updates++;
				output::doOutput("Password value has been updated.`n");
			...
			if ((int)$value != (int)$oldvalues['superuser']){
				$sql.="$key = \"$value\",";
				$updates++;
				output::doOutput("Superuser values have changed.`n");
				if ($session['user']['acctid']==$userid) {
			...
			if ($tmp != stripslashes($val)) {
				output::doOutput("`\$Illegal characters removed from player name!`0`n");
			}
			if (soap($tmp) != ($tmp)) {
				output::doOutput("`^The new name doesn't pass the bad word filter!`0");
			}
				$newname = change_player_name($tmp, $oldvalues);
			$sql.="$key = \"".addslashes($newname)."\",";
			output::doOutput("Changed player name to %s`0`n", $newname);
			...
			$tmp = sanitize_html($tmp);
			if ($tmp != stripslashes($val)) {
				output::doOutput("`\$Illegal characters removed from player title!`0`n");
			}
			if (soap($tmp) != ($tmp)) {
				output::doOutput("`^The new title doesn't pass the bad word filter!`0");
			}
				$newname = change_player_title($tmp, $oldvalues);
			$sql.="$key = \"$val\",";
			output::doOutput("Changed player title from %s`0 to %s`0`n", $oldvalues['title'], $tmp);
			$oldvalues[$key]=$tmp;
			if ($newname != $oldvalues['name']) {
				$sql.="name = \"".addslashes($newname)."\",";
				output::doOutput("Changed player name to %s`0 due to changed dragonkill title`n", $newname);
			...
			$tmp = sanitize_html($tmp);
			if ($tmp != stripslashes($val)) {
				output::doOutput("`\$Illegal characters removed from custom title!`0`n");
			}
			if (soap($tmp) != ($tmp)) {
				output::doOutput("`^The new custom title doesn't pass the bad word filter!`0");
			}
			$newname = change_player_ctitle($tmp, $oldvalues);
			$sql.="$key = \"$val\",";
			output::doOutput("Changed player ctitle from %s`0 to %s`0`n", $oldvalues['ctitle'], $tmp);
			$oldvalues[$key]=$tmp;
			if ($newname != $oldvalues['name']) {
				$sql.="name = \"".addslashes($newname)."\",";
				output::doOutput("Changed player name to %s`0 due to changed custom title`n", $newname);
			...
			$sql.="$key = \"$val\",";
			$updates++;
			output::doOutput("%s has changed to %s.`n", $key, stripslashes($val));
...
output::addnav("","user.php");
	...
	output::doOutput("%s fields in the user's record were updated.", $updates);
}else{
	output::doOutput("No fields were changed in the user's record.");
}
$op = "edit";
httpset($op, "edit");
