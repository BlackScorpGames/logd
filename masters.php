...
if ($op == "del") {
	...
	db_query($sql);
	output::doOutput("`^Master deleted.`0");
	$op = "";
	httpset("op", "");
} elseif ($op == "save") {
	...
	db_query($sql);
	if ($id == 0) {
		output::doOutput("`^Master %s`^ added.", stripslashes($name));
	} else {
		output::doOutput("`^Master %s`^ updated.", stripslashes($name));
	}
	$op = "";
	httpset("op", "");
} elseif ($op == "edit") {
	output::addnav("Functions");
	output::addnav("Return to Masters Editor", "masters.php");
	...
	output::addnav("","masters.php?op=save&id=$id");
	rawoutput("<form action='masters.php?op=save&id=$id' method='POST'>");
	output::doOutput("`^Master's level:`n");
	...
	output_notl("`n");
	output::doOutput("`^Master's speech when player wins:`n");
	...
	output_notl("`n");
	output::doOutput("`^Master's speech when player loses:`n");
	...
	rawoutput("</form>");
	output_notl("`n`n");
	output::doOutput("`#The following codes are supported in both the win and lose speeches (case matters):`n");
	output::doOutput("%w = The players's name (can be specified as {goodguy}`n");
	output::doOutput("%W = The masters's name (can be specified as {badguy}`n");
	output::doOutput("%x = The players's weapon (can be specified as {weapon}`n");
	output::doOutput("%X = The master's weapon (can be specified as {creatureweapon}`n");
	output::doOutput("%a = The players's armor (can be specified as {armor}`n");
...
if ($op == "") {
	output::addnav("Functions");
	output::addnav("Refresh list", "masters.php");
	output::addnav("Add master", "masters.php?op=edit&id=0");
	...
	while ($row = db_fetch_assoc($res)) {
		...
		output_notl($del);
		rawoutput("] </a>");
		output::addnav("","masters.php?op=edit&id=$id");
		output::addnav("","masters.php?op=del&id=$id");
		rawoutput("</td><td>");
	...
	rawoutput("</table>");
	output::doOutput("`n`#You can change the names, weapons and messages of all of the Training Masters.");
	output::doOutput("It is suggested, that you do not toy around with this, unless you know what you are doing.`0`n");
}
page_footer();
